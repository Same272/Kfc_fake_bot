...
def delete_user_cart(user_id):
    connection = sqlite3.connect("fake_kfc.db")
    sql = connection.cursor()
    all_cart_products_delete= sql.execute("DELETE FROM cart;", (user_id, )).fetchall()
    return all_cart_products_delete# укажите параметры

# получение информации для меню корзины в формате
# (Название продукта ; количество ; цена)
def get_user_cart(user_id): # укажите параметры
    connection = sqlite3.connect("fake_kfc.db")
    sql = connection.cursor()
    all_cart_products = sql.execute("SELECT * FROM cart;", (user_id, )).fetchall()
